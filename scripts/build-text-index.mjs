 ...
 * a wrong answer. Stories, tests and src/app/api (server routes, whose
 * strings never reach a node anyone can pick) are skipped for the same
 * reason.
 *
 * It also emits MODULE_PATHS, the map from a stylesheet's basename to where
 * that file sits, which is what lets a STYLES row link (see the emit).
 *
...
/* Server routes never put a node on the desktop, so a string of theirs can
   only ever be a wrong answer to "where do these words live". Their error
   messages read like sentences and passed the prose filter on that alone,
   which is how "github commit failed" got into a table meant for the
   things a visitor can point at. */
const SKIP_PATH = new Set([path.join(ROOT, 'src/app/api')])
...
function filesUnder(dir, ext) {
  ...
  const walk = (at) => {
    for (const name of readdirSync(at)) {
      ...
      if (SKIP_PATH.has(full)) continue
...
/* ---- MODULE PATHS: a stylesheet's name back to the file it came from ----
 *
 * The same closing-the-loop-at-build-time move the strings above take.
 * INSPECT reads a module stylesheet's NAME off the class the bundler wrote
 * (File_class__hash) and has no way to learn where that file sits, so the
 * SOURCE row could print `dock.module.css` and stop there. The folder is
 * knowable here, so it ships, and the row becomes a link.
 *
 * Keyed by basename, which is what the class prefix spells. The one shape
 * that would miss is an index.module.css, whose classes are prefixed with
 * the FOLDER name instead: there are none today, and the miss is quiet
 * (the row falls back to the bare name it always printed).
 *
 * The value is a list because two folders may hold the same basename. The
 * panel prints every candidate rather than picking one.
 */
const modules = new Map()
for (const file of filesUnder(path.join(ROOT, 'src'), '.module.css')) {
  const name = path.basename(file)
  const seen = modules.get(name)
  if (!seen) modules.set(name, [rel(file)])
  else if (!seen.includes(rel(file))) seen.push(rel(file))
}

const moduleKeys = Array.from(modules.keys()).sort()
const moduleBody = moduleKeys
  .map((k) => `  ${JSON.stringify(k)}: ${JSON.stringify(modules.get(k))},`)
  .join('\n')
...
console.log(
  `text index: ${keys.length} strings from ${files.size} files, ` +
    `${moduleKeys.length} module stylesheets, ` +
