...
/* What My AI Thinks — the spec sheet plus the four-section appraisal that
   used to be folded into a twirl-down at the bottom of ABOUT THIS MACHINE.
   Same words, now given a document to live in: one h1, h2 section heads,
   a comfortable measure, and a window that scrolls.

   Sources for the appraisal: LinkedIn, Jake's 2024/2025 Lattice reviews
   (manager + peer), and verbatim user research from the Invest study.
   Written by Claude and signed as such — Jake asked for honest, so it is. */
...
const SPECS: Array<[string, string]> = [
  ...
  ['Current post', 'Staff Product Designer, Greenlight'],
]

export default function AiOpinion() {
  return (
    <div className={styles.opinion}>
      <header className={styles.opinionHead}>
        <Copy k="machine.ai.eyebrow" as="p" className={styles.opinionEyebrow} />
        <Copy k="machine.ai.title" as="h1" className={styles.opinionTitle} />
      </header>

      <Copy k="machine.essayNote" as="p" className={styles.essayNote} />

      <Copy k="machine.ai.specsHeading" as="h2" className={styles.specsHeading} />
